...
colxi = {c: xi[[v for v in G.nodes if xi[v]==c][0]] for c in B.nodes}
...
qf.graphs.save(Gp, args.output_basename + "-repaired.dot", args.output_basename + "-repaired.png", colors=gt)

# Compute and save difference
Gdif = qf.graphs.difference(Gp, G)
qf.graphs.save(Gdif, args.output_basename + "-diff.dot", args.output_basename + "-diff.png", colors=gt, labelNodes=True, labelArcs=False)
qf.graphs.save(B, args.output_basename + "-base.dot", args.output_basename + "-base.png", colors=colxi, labelNodes=False, labelArcs=False)
